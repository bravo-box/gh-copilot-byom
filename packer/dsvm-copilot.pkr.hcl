...
#   }
  ...
  provisioner "powershell" {
    inline = [
      ...
      "",
      "# Redirect npm global prefix to a machine-wide location so packages",
      "# survive Sysprep (the default %APPDATA%\\npm lives in the build user",
      "# profile which is removed during generalisation).",
      "$npmGlobalDir = 'C:\\npm-global'",
      "New-Item -ItemType Directory -Path $npmGlobalDir -Force | Out-Null",
      "npm config set prefix $npmGlobalDir",
      "Write-Host \">>> npm global prefix set to: $npmGlobalDir\"",
      "",
      "npm install -g @github/copilot",
      "Write-Host '>>> npm global prefix:' (npm prefix -g)",
      "Write-Host '>>> Copilot CLI version:'",
      "& \"$npmGlobalDir\\copilot\" --version",
      "",
      "# Add npm global bin directory to the SYSTEM PATH so it survives Sysprep",
      "Write-Host \">>> Adding npm global bin to system PATH: $npmGlobalDir\"",
      "$machinePath = [System.Environment]::GetEnvironmentVariable('Path', 'Machine')",
      "if ($machinePath -notlike \"*$npmGlobalDir*\") {",
